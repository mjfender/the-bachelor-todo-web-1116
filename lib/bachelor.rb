...
def get_first_name_of_season_winner(data, season)
  ...
  data.each do |seasons, contestants|
    contestants.each do |attributes|
      #binding.pry
      #puts "#{season} and #{attributes[0][:name]} and #{attributes[:status]}"
      if season == seasons && attributes["status"] == "Winner"
...
#get_first_name_of_season_winner(bachelor_data, "season 17")
...
def get_occupation(data, hometown)
  ...
  data.each do |seasons, contestants|
    contestants.each do |attributes|
      if attributes["hometown"] == hometown && one_and_done == false
